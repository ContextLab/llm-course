...
def generate_flow_svg(flow_lines: list, caption: str = None) -> str:
    ...
    for node in all_nodes:
        if node["content"]["color"] is None:
            node["content"]["color"] = DEFAULT_COLOR_SEQUENCE[
                color_idx % len(DEFAULT_COLOR_SEQUENCE)
            ]
            color_idx += 1
    ...
    for row_elements in all_elements:
        row_nodes = [e for e in row_elements if e["type"] == "node"]
        num_arrows = len(
            [e for e in row_elements if e["type"] in ("arrow_h", "arrow_v")]
        )
    ...
    for row_idx, row_elements in enumerate(all_elements):
        ...
        for elem in row_elements:
            if elem["type"] == "node":
                ...
                svg_parts.append(
                    f'        fill="{colors["fill"]}" stroke="{colors["stroke"]}" stroke-width="3"/>'
                )
...
TWO_COLUMN_PATTERN = re.compile(
    r'<div[^>]*style="[^"]*display:\s*flex[^"]*".*?</div>\s*</div>', re.DOTALL
)
    ...
    stats = {
        ...
        "overflow_warnings": len(
            [
                ...
                if "overflow" in w.lower() or "exceeds" in w.lower()
            ]
        ),
        "scale_classes_injected": len(
            [w for w in overflow_warnings if "Auto-injecting" in w]
        ),
    }
...
def main():
    ...
    parser.add_argument(
        "--no-split", action="store_true", help="Disable code block and table splitting"
    )
    ...
    try:
        ...
        print(
            f"Processed: {stats['input_lines']} input lines -> {stats['output_lines']} output lines"
        )
