...
\begin{frame}{Part-of-Speech (POS) Tagging 🎯}
    ...
    \begin{center}
        \begin{tikzpicture}
            \node[font=\small] at (0,1) {The};
            \node[font=\small] at (1.2,1) {cat};
            \node[font=\small] at (2.4,1) {sat};
            \node[font=\small] at (3.6,1) {on};
            \node[font=\small] at (4.8,1) {the};
            \node[font=\small] at (6,1) {mat};
            ...
            \node[font=\small, color=darkblue] at (0,0) {DET};
            \node[font=\small, color=darkgreen] at (1.2,0) {NOUN};
            \node[font=\small, color=orange] at (2.4,0) {VERB};
            \node[font=\small, color=purple] at (3.6,0) {ADP};
            \node[font=\small, color=darkblue] at (4.8,0) {DET};
            \node[font=\small, color=darkgreen] at (6,0) {NOUN};
        \end{tikzpicture}
    \end{center}
...
\begin{frame}{POS Tagsets: Universal vs. Fine-Grained 📋}
    \begin{columns}[T]
...
\begin{frame}{How Do POS Taggers Work? 🧠}
...
\begin{frame}{Neural Networks and Grammar 🤖}
    ...
    \begin{center}
        \begin{tikzpicture}
            \node[align=left, font=\small] at (0,2) {
                "The key to the cabinets \textcolor{darkgreen}{\textbf{is}} on the table" ✅
            };
            \node[align=left, font=\small] at (0,1) {
...
\begin{frame}{Discussion: Do Models "Understand" Grammar? 🤔}
...
\begin{frame}{Sentiment Analysis: From Structure to Emotion 😊😢}
...
\begin{frame}{Sentiment Analysis Challenges 🚧}
...
\begin{frame}{Modern Approach: Pre-trained Models 🤖}
    ...
    \begin{center}
        \begin{tikzpicture}[node distance=1.5cm]
            ...
            \node[rectangle, draw, fill=orange!30, minimum width=2.5cm, below of=input] (bert) {Pre-trained Model\\(BERT, RoBERTa)};
            \node[rectangle, draw, fill=darkgreen!30, minimum width=2.5cm, below of=bert] (cls) {Classification Head\\(Dense + Softmax)};
...
\begin{frame}{Fine-Tuning for Sentiment Analysis 🎓}
...
\begin{frame}{Evaluation Metrics for Sentiment Analysis 📏}
...
\begin{frame}{Confusion Matrix Example 🎭}
    \begin{center}
        \begin{tikzpicture}
            ...
            % Labels
            \node[font=\small] at (1,4.5) {Predicted Negative};
            \node[font=\small] at (3,4.5) {Predicted Positive};
            \node[font=\small, rotate=90] at (-0.7,3) {Actual Negative};
            \node[font=\small, rotate=90] at (-0.7,1) {Actual Positive};

            % Values
            \node[font=\Large] at (1,3) {850};
            \node[font=\small, below] at (1,2.5) {True Negative};

            \node[font=\Large] at (3,3) {50};
            \node[font=\small, below] at (3,2.5) {False Positive};

            \node[font=\Large] at (1,1) {100};
            \node[font=\small, below] at (1,0.5) {False Negative};

            \node[font=\Large] at (3,1) {900};
            \node[font=\small, below] at (3,0.5) {True Positive};
...
\begin{frame}{Aspect-Based Sentiment Analysis 🔍}
...
\begin{frame}{Real-World Application: Product Review Analysis 🛍️}
...
\begin{frame}{Discussion: Understanding Emotion 💭}
...
% Week 2 Wrap-up
\begin{frame}{Week 2 Integration: Putting It All Together 🧩}
...
\begin{frame}{Assignment 2: SPAM Email Classifier 📧}
...
\begin{frame}{Looking Ahead: Weeks 3-4 🔮}
    \textbf{Next topics:}
    \begin{itemize}
