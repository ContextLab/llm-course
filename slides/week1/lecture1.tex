...
\begin{frame}{Types of Consciousness 🎯}
\begin{center}
...
\node[below, align=center] at (0,-2.5) {Subjective\\Experience};
\node[below, align=center] at (2.5,-2.5) {Information\\Processing};
\node[below, align=center] at (1.25,-2.5) {Full\\Consciousness};
\end{tikzpicture}
\end{center}
...
\begin{frame}{The Chinese Room Argument 🏮}
\begin{columns}
\begin{column}{0.5\textwidth}
...
\node at (2,4.3) {\textbf{Chinese Room}};
...
% Input/Output
\draw[->, thick, red] (-0.5,3) -- (0,3) node[midway, above] {中文};
\draw[->, thick, green] (4,1) -- (4.5,1) node[midway, above] {中文};
\end{tikzpicture}
\end{center}
...
\begin{frame}{The Language-Thought Spectrum 📊}
\begin{center}
\begin{tikzpicture}[scale=1.0]
% Spectrum line
...
% Left extreme
\node[above, align=center, font=\small] at (0,0.3) {\textbf{Language = Thought}\\No language,\\no thought};
\node[below, font=\tiny] at (0,-0.5) {Strong Whorfian};

% Right extreme
\node[above, align=center, font=\small] at (10,0.3) {\textbf{Language $\neq$ Thought}\\Thought exists\\independently};
\node[below, font=\tiny] at (10,-0.5) {Language-of-Thought};

% Middle positions
\node[above, align=center, font=\small] at (3,0.3) {Language\\shapes thought};
\node[below, font=\tiny] at (3,-0.5) {Weak Whorfian};

\node[above, align=center, font=\small] at (7,0.3) {Thought\\influences\\language};
...
\begin{frame}{The Russian Blue Example 🎨}
\begin{center}
\begin{tikzpicture}[scale=1.0]
% English speakers
\node at (0,3) {\textbf{English Speakers}};
\draw[fill=blue!30] (0,2) rectangle (2,2.5);
\node[below] at (1,2) {"blue"};

% Russian speakers
\node at (6,3) {\textbf{Russian Speakers}};
\draw[fill=blue!70] (5,2) rectangle (6,2.5);
\node[below, font=\small] at (5.5,2) {"siniy" (dark blue)};
\draw[fill=blue!30] (6.2,2) rectangle (7.2,2.5);
\node[below, font=\small] at (6.7,2) {"goluboy" (light blue)};

% Arrow showing difference
\draw[thick, <->, red] (2.5,2.25) -- (4.5,2.25);
\node[red, above] at (3.5,2.25) {Different categories!};

% Perception effect
\node[below, align=center] at (1,0.5) {Slower to\\distinguish\\shades};
\node[below, align=center] at (6,0.5) {Faster to\\distinguish\\shades};
\end{tikzpicture}
\end{center}
...
\begin{frame}{The Grounding Problem 🌍}
\begin{columns}
\begin{column}{0.5\textwidth}
...
% Human grounding
\node at (0,4) {\textbf{Human}};
\node[draw, circle, fill=blue!20] (word) at (0,3) {Word};
\node[draw, circle, fill=green!20] (exp) at (0,1.5) {Experience};
\draw[<->, very thick] (word) -- (exp);
\node[right, font=\tiny] at (0.2,2.25) {grounded};

% LLM
\node at (3.5,4) {\textbf{LLM}};
\node[draw, circle, fill=blue!20] (word2) at (3.5,3) {Word};
\node[draw, circle, fill=red!20] (other) at (3.5,1.5) {Other\\Words};
...
\begin{frame}{From Simple to Complex 📈}
\begin{center}
\begin{tikzpicture}[scale=0.9]
% Axes
\draw[->] (0,0) -- (10,0) node[right] {Course Progress};
\draw[->] (0,0) -- (0,5) node[above] {Complexity};
...
\node[right, red, font=\small] at (10,4.5) {Understanding?};
\end{tikzpicture}
\end{center}
...
\begin{frame}{HuggingFace: Your Learning Companion 🤗}
...
\begin{itemize}
    \item 📚 \textbf{Course materials}: Free NLP course (\href{https://huggingface.co/course}{huggingface.co/course})
...
\begin{frame}{Next Lectures This Week 🔮}
